  ...
  if (layout === "back_cover") {
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-back_cover" style="width:100%;height:100%;overflow:hidden;position:relative;">
        <img src="/images/babbl-book.svg" alt="Back Cover" class="back-cover" />
      </div>
    </div>`;
  }
  ...
  if (layout === "circle_photo") {
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-circle_photo" style="width:100%;height:100%;overflow:hidden;position:relative;">
        <div class="bg-image">${photoUrl ? `<img crossorigin="anonymous" src="${photoUrl}" alt="Background" />` : `<span>This Babbl doesn't have an image.</span>`}</div>
    ...
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-quote_top_photo_bottom" style="width:100%;height:100%;overflow:hidden;position:relative;">
        <div class="quote-section">
          ${avatarUrl ? `<div class="avatar-container"><img crossorigin="anonymous" src="${avatarUrl}" alt="Child" /></div>` : ""}
          <div class="quote-text"><h3>"${q.text}"</h3><div class="author-details">${childName()}${childAge()}</div>${!(q.context && page.show_context !== false) ? `<div class="meta-date">${photoDate()}</div>` : ""}</div>
        </div>
  ...
  if (layout === "full_page_photo_quote_centered") {
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-full_page_photo_quote_centered" style="width:100%;height:100%;overflow:hidden;position:relative;">
        <div class="author-pill">${avatarUrl ? `<div class="author-avatar"><img crossorigin="anonymous" src="${avatarUrl}" alt="Child" /></div>` : ""}<span class="author-name">${childName()}${childAge()}</span></div>
  ...
  if (layout === "full_width_photo_top_quote_bottom") {
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-full_width_photo_top_quote_bottom" style="width:100%;height:100%;overflow:hidden;position:relative;">
        <div class="image-section">${photoUrl ? `<img crossorigin="anonymous" src="${photoUrl}" alt="Photo" />` : `<span>No image available.</span>`}</div>
        <div class="quote-section"><h3>"${q.text}"</h3><span class="meta-date">${photoDate()}</span><div class="author-row"><div class="author-details">${childName()}${childAge()}</div>${avatarUrl ? `<div class="author-avatar"><img crossorigin="anonymous" src="${avatarUrl}" alt="Child" /></div>` : ""}</div></div>
      </div>
    </div>`;
  }
    ...
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-full_screen_photo_short_quote" style="width:100%;height:100%;overflow:hidden;position:relative;">
        <div class="quote-section">
          ...
          <div class="author-pill"><span class="author-name">${childName()}${childAge()}</span>${avatarUrl ? `<div class="author-avatar"><img crossorigin="anonymous" src="${avatarUrl}" alt="Child" /></div>` : ""}</div>
        </div>
        <div class="image-section">${photoUrl ? `<img crossorigin="anonymous" src="${photoUrl}" alt="Photo" />` : `<span>No image available.</span>`}</div>
      </div>
    </div>`;
    ...
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-photo_window_top_quote_bottom" style="width:100%;height:100%;overflow:hidden;position:relative;">
        ...
        <div class="quote-card">
          <div class="quote-text"><h3>"${q.text}"</h3><div class="meta-row"><span class="author-date">${photoDate()}</span></div></div>
  ...
  if (layout === "quote_only_centered") {
    return `<div class="print-page theme-babbl_theme" style="${containerStyle}">
      <div class="page-layout layout-quote_only_centered" style="width:100%;height:100%;overflow:hidden;position:relative;">
        ${avatarUrl ? `<div class="avatar-container"><img crossorigin="anonymous" src="${avatarUrl}" alt="Child" /></div>` : ""}
        <div class="quote-section"><h3>"${q.text.trim()}"</h3><div class="author-details">${childName()}${childAge()}</div></div>
...
export default define.page(async function PrintBook(ctx) {
  ...
  const isAdminBypass = (serviceRoleKey && token === serviceRoleKey) ||
  ...
  // Render all pages as HTML strings
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Aleo:wght@700&family=Rosario:wght@400;700&family=Yomogi&family=Fredoka:wght@400;500;600;700&family=Charter:ital,wght@0,400;0,700;1,400;1,700&display=swap" rel="stylesheet" />
